...
// This is a canary value so we can confirm whether this decoder has booted
// before
...
// Calculate the flash address where we will store channel info as the 2nd to
// last page available
#define FLASH_STATUS_ADDR                                                      \
 ...
 */
int update_subscription(pkt_len_t pkt_len,
                        subscription_update_packet_t *update) {
  int i;

  if (update->channel == EMERGENCY_CHANNEL) {
    STATUS_LED_RED();
    print_error("Failed to update subscription - cannot subscribe to emergency "
                "channel\n");
    return -1;
  }
  ...
  if (i == MAX_CHANNEL_COUNT) {
    STATUS_LED_RED();
    print_error(
        "Failed to update subscription - max subscriptions installed\n");
    return -1;
  }
...
void init() {
  ...
  if (decoder_status.first_boot != FLASH_FIRST_BOOT) {
     ...
     * Whenever the decoder processes a subscription update, this data will be
     * updated.
     */
