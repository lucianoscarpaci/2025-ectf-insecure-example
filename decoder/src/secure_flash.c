 ...
 * 
 * This ISR allows for access to the flash through secure_flash to operate
 */
void flash_secure_irq(void) {
    uint32_t temp;
    temp = MXC_FLC0->intr;
...
*/
void flash_secure_init(void) {
    // Setup Flash
    MXC_NVIC_SetVector(FLC0_IRQn, flash_secure_irq);
...
*/
int flash_secure_erase_page(uint32_t address) {
...
*/
void flash_secure_read(uint32_t address, void* buffer, uint32_t size) {
 ...
 * flash_secure_erase_page documentation.
*/
int flash_secure_write(uint32_t address, void* buffer, uint32_t size) {
